...
You are planning to travel to another country for vacation, so you make it today's goal to convert your USD (all bills,
no cents) into the appropriate currency. This will help you be more prepared for when you arrive in the country you will
be vacationing in.

Given an integer (usd) representing the amount of dollars you have and a string (currency) representing the name of the
currency used in another country, it is your task to determine the amount of foreign currency you will receive when you
exchange your United States Dollars.
...
def convert_my_dollars(usd, currency)
  # use CONVERSION_RATES hash
  rate = CONVERSION_RATES[currency]
  first_letter = currency[0].downcase
  
  if ['a', 'e', 'i', 'o', 'u'].include?(first_letter)
    amount = rate * usd
  else
    amount = rate.to_s().to_i(2) * usd # Convert from binary string to integer
  end
  "You now have #{amount} of #{currency}."
end
